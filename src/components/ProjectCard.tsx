...
export function ProjectCard({ title, image, tags, description, link, github }: Props) {
	return <article class="group relative bg-zinc-800 rounded-md shadow-md overflow-hidden" tabIndex={0}>
		<div class="flex p-1 items-center">
			...
			{link && <a href={link} target="_blank" aria-label="External link" class="p-1 fill-current hover:fill-zinc-300">{Icons.external_link}</a>}
			{github && <a href={github} target="_blank" aria-label="GitHub repository" class="p-1 fill-current hover:fill-zinc-300">{Icons.github}</a>}
		</div>
		<div class="flex py-1 px-2 gap-1">
			{tags?.map(tag => <Tag name={tag} />)}
		</div>
		<div class="relative aspect-video bg-zinc-700 cursor-pointer overflow-hidden">
			<img src={image} alt={`Project image for "${title}"`} class="aspect-video object-cover motion-safe:transition-all group-focus:brightness-[25%] group-focus:scale-150 group-hover:brightness-[25%] group-hover:scale-150" />
